...
public class MailSenderScheduler {
    ...
    // Runs every minute
    @Scheduled(cron = "0 * * * * ?") // will it be 13:20:000 or seconds will be not 0??????????????????????????????
    // @ToDo. Think about this because evey min we will make db lookups (maybe redis for this day and update if new)
    ...
    public void sendDailyTaskNotifications() {
        ...
        // filter tasks for time if its 13:20 we should send all tasks from 13:19:01 to 13:20:00
